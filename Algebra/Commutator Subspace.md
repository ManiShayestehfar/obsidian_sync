...
>2. $A = \text{Mat}_d(\mathbb{F})$, then $[A,A]= \{a \in A \:|\: \text{tr}(a) = 0\} = \mathfrak{sl}(\mathbb{F})$ and $\dim A / [A,A] = 1$.
...
>4. If $\mathcal{O}$ is a *commutative* division ring over $\mathbb{F}$, then $[\text{Mat}_n(\mathcal{O}), \text{Mat}_n(\mathcal{O})] = \mathfrak{sl}_n(\mathcal{O})$.
