...
# Number of Irreducible A-modules
...
Now consider $AV = \{av:a \in A, v \in V\}$. This is a subspace of $V$ generated by all $A$-actions. But $V$ is already built up from $A$. 
Hence $V = AV = D_1V + D_2V + \cdots + D_tV$ where each $D_iV = \{d_i\cdot v:d_i \in D_i, v \in V\}$ is a submodule of $V$.
...
Hence we showed that $V = \sum_{i,a}D_ie_a$, BUT this is not necessarily direct. 
Suppose $D,D'$ are distinct irreducible submodules of $V$. Then either $D=D'$, or $D \cap D' = \{0\}$. So we can refine the $D_ie_a$ pieces into a direct sum,
