...
- By induction assume the claim for $e_i$. Therefore $e_{i+1}x=(3e_i^2-2e_i^3)x= (3-2)x$ and $$\begin{align*}
e_{i+1} - e_{i+1}^{2}
&= (2e_i^{2} - 3e_i^{3})(1 - 3e_i^{2} + 2e_i^{3}) \\
&= (2e_i^{2} - 3e_i^{3})(1 - 2e_i + e_i^{2})(1 + 2e_i) \\
&= (2e_i^{2} - 3e_i^{3})(1 + 2e_i)(1 - e_i)^{2} \\
&= (2 - 3e_i)(1 + 2e_i)(e_i - e_i^{2})^{2} \\
&\in (2 - 3e_i)(1 + 2e_i)N^{2^{i+1}}.
\end{align*}$$where the last line follows by induction since $e_i-e_i^2 \in N^{2i}$. This completes the proof of claim. $\square$

Now we are done because $e_i\neq0$ since $x=e_ix$. On the other hand, $N$ is nilpotent, so $N^k=0$ for some $k>0$. In particular $N^{2^i}=0$ for some $i >0$ so $e_i-e_i^2=0$ for $i >0$. That is, $e_i=e_i^2$ is an idempotent (and $e_i=e_{i+1}$) as soon as $N^{2^i}=0$.  $\square$
...
##### Proof
###### 1)
Take $r \in \text{Rad }A$ and $a \in A$. Since $\text{Rad }A$ is a left-ideal, then $ar \in \text{Rad }A$. 
Write $(ra)^n = r(ar)^{n-1}a$.
Since $r,ar \in \text{Rad }A$, then $r(ar)^{n-1}\in (\text{Rad }A)^n$.
So $\text{Rad }A$ is nilpotent, with say $(\text{Rad }A)^n=0$, it follows that 
$$(ra)^n = r(ar)^{n-1}a =0.$$
Thus $ra$ must be nilpotent. Therefore $A(ra)$ is nilpotent, and by maximality of $\text{Rad }A$, $A(ra) \subseteq R$. Hence $ra \in R$, and therefore $\text{Rad }A$ is also a right-ideal.
