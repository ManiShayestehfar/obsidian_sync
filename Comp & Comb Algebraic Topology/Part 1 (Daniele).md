...
Let $\mu: E(G) \to \mathbb{C}[t^{\pm1/2}]$ be a polynomial and define $G_\mu := (G,\mu)$ a weighted graph. Then the **partition function** for $G_\mu$ is 
$$\mathcal{Z}(G_\mu) = \sum_{m \in M(G)} \prod_{e \in m} \mu(e)$$

- If $G$ is a planar and bipartite graph (e.g. $V(G) = V_1 \sqcup V_2$) and only consider the perfect matchings (e.g. $|V_1| = |V_2|$)
- A famous example (see page 31) is $\text{knots} \to \mathbb{Z}[t^{\pm 1/2}]$ the alexander polynomial is defined such that $\Delta_0(t) = 1$, and $\Delta_{K_+}(t)- \Delta_{K_-}(t)= (t^{1/2} - t^{-1/2})\Delta_{K_0}(t)$



# Discrete Morse Theory

Let $M^n$ be an $n$-dim smooth, oriented, closed manifold in $\mathbb{R}^N$. Let $f:M^n \to \mathbb{R}$ be a "generic" function (i.e. only isolated singularities and outside of these critical values $f^{-1}$ is a submanifold)

## Theorem

Locally around a critical point $c$
$$f(p) = p + \sum_{i=1}^k x_i^2 - \sum_{j=k+1}^n x_i^2$$
where $k$ is the index of $c$.


## Morse Homology

$C_k$ are generated by critical points of index $k$.
$\partial$ "counts the number of flowlines"

![[Part 1 (Daniele)-1771157086309.png]]



## Discrete Morse Matching

$X$ a scpx and $m \in M(X)$. 
1. Swap the orientation of all the edges of $\mathcal{F}(X)$ that belong to $m$. 
2. If this does NOT create oriented cycles, then $m$ is a **discrete Morse matching**

![[Part 1 (Daniele)-1771157190265.png]]
...
Let $m \in \mathcal{M}(X)$. A simplex $\sigma \in X$ is **critical** if it is not matched.

![[Part 1 (Daniele)-1771157580713.png]]


## Discrete Morse Homology

Fix $m \in \mathcal{M}(X)$. 
- Generators of $C_k(X,m; \mathbb{Z}_2)$ are critical $k$-simplices. 
