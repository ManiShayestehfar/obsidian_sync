...
Changing basepoint of $\pi_1(\widetilde{X})$ from $\widetilde{x}_1$ to $\widetilde{x}_2$ corresponds to conjugating $H$ by $[\gamma]\in \pi_1 (X,x_0)$ where $\gamma$ lifts to a path $\widetilde{\gamma}$ from $\widetilde{x}_1$ to $\widetilde{x}_2$. So $[\gamma]\in N_G(H) \iff p_*(\pi_1 (\widetilde{X},\widetilde{x}_1)) = p_*(\pi_1 (\widetilde{X},\widetilde{x}_2)) \iff \exists \:\text{lifts}\: \widetilde{f}:(\widetilde{X},\widetilde{x}_1) \to (\widetilde{X},\widetilde{x}_2)$ and $\widetilde{g}:(\widetilde{X},\widetilde{x}_2) \to (\widetilde{X},\widetilde{x}_1)$ such that $p \circ \widetilde{f} = p$ and $p \circ \widetilde{g}=p$ (from lifting criterion).
